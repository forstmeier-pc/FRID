...
def scan(myfasta, s=10, w=120, r=50, t=37, type='mono', prt ='off', print_rand ='off'):

    myfasta = myfasta
    step_size = int(s)
    window_size = int(w)
    randomizations = int(r)
    temperature = int(t)
    type = str(type)
    print_to_screen = str(prt)
    print_random = str(print_rand)

    w = open(myfasta.replace('.fasta',"sf_scan.txt"), 'w')
    #####+".forward.win_"+str(window_size)+".stp_"+str(step_size)+".rnd_"+str(randomizations)+".shfl_"+str(type)+".txt", 'w')
    md = RNA.md()
        ...
        for cur_record in SeqIO.parse(forward_fasta, "fasta") :
            ...
            while i == 0 or i <= (length - window_size):
                ...
                if -1 == 0:
                    ...
                    #### THIS IS WHERE I PUT IN THE I OR J IN SPACER CONDITIONAL

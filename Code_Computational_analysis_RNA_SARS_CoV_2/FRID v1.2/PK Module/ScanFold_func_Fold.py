...
## ##
                ...
                elif ("U") in str(data[7]):
                    sequence = transcribe(str(data[7]))
                elif ("G") in str(data[7]):
                    sequence = transcribe(str(data[7]))
                elif ("C") in str(data[7]):
                    sequence = transcribe(str(data[7]))
                elif ("N") in str(data[7]):
                    data[7] = str(data[7])
                    data[7].replace('N', 'A')
                    sequence = transcribe(str(data[7]))
                else:
...
def write_ct(base_pair_dictionary, filename, filter, strand, start_coordinate):
...
def write_bp(base_pair_dictionary, filename, minz):
    w = open(filename, 'w')
...
def fold(filename, filter, competition):
        ...
        for row in lines:
            ...
            #Main loop to find all i-j pairs per i-nucleotide
                ...
                while l < len(base_pairs):
                    ...
                    #finds the location of the bases relative to the window
                    lbp = base_pairs[l]
                    rbp = base_pairs[l+1]

                    #finds the actual letters of the bases
                    lb = str(sequence[int(lbp)-1])
                    rb = str(sequence[int(rbp)-1])

                    #finds the location of the bases in the overall sequence
    ...
    for k, v in sorted(bp_dict.items()):
        #k is the i-nuc coordinate
        #v is the set of pairs that could exist for the i -nuc
        ...
        #total_windows is telling us how many windows there are with a pair appearing in it for each nucl
        #num_bp is telling up how amny of them are not paired with themselves
    ...
    if competition == 0:
        ...
        #Write bp files
        write_bp(best_bps, "best_bps_test.bp", minz)
    ...
    if competition == 1:
        ...
        if filter != None or filter != -2:
            write_ct(final_partners, output+str(filter)+".ct", filter, strand, start_coordinate)
        write_ct(final_partners, output+"no_filter.ct", float(10), strand, start_coordinate)
        #print('')
        write_ct(final_partners, output+"-1.ct", float(-1), strand, start_coordinate)
        write_ct(final_partners, output+"-2.ct", float(-2), strand, start_coordinate)
        #write_ct(final_partners, output+"-2.1.ct", float(-2.1), strand, start_coordinate)
        #write_ct(final_partners, output+"-2.2.ct", float(-2.2), strand, start_coordinate)
        #write_ct(final_partners, output+"-2.3.ct", float(-2.3), strand, start_coordinate)
        write_ct(final_partners, output+"-2.41.ct", float(-2.41), strand, start_coordinate)
        #write_ct(final_partners, output+"-2.5.ct", float(-2.5), strand, start_coordinate)
        #write_ct(final_partners, output+"-2.6.ct", float(-2.6), strand, start_coordinate)
        #write_ct(final_partners, output+"-2.7.ct", float(-2.7), strand, start_coordinate)
        #write_ct(final_partners, output+"-2.8.ct", float(-2.8), strand, start_coordinate)
        #write_ct(final_partners, output+"-2.9.ct", float(-2.9), strand, start_coordinate)
        #write_ct(final_partners, output+"-3.ct", float(-3), strand, start_coordinate)

        write_ct(final_partners, output+"below_mean_"+str(round(meanz, 2))+".ct", meanz, strand, start_coordinate)
        write_ct(final_partners, output+"1sd_below_mean_"+str(round(one_sig_below, 2))+".ct", one_sig_below, strand, start_coordinate)
        write_ct(final_partners, output+"2sd_below_mean_"+str(round(two_sig_below, 2))+".ct", two_sig_below, strand, start_coordinate)
        ...
        write_bp(final_partners, "final_partners_test.bp", minz)
        print("ScanFold-Fold complete, find results in...")
